...
public class CommentServiceImpl implements CommentService {
    ...
    public List<CommentResponseDto> getCommentByUser(Long userId) { // 특정 유저가 작성한 댓글 조회
        User user = userRepository.findById(userId).orElseThrow(
            () -> new NotFoundException("해당 유저는 존재하지 않습니다.")
        );
    ...
    @Override
    @Transactional
    public CommentResponseDto createComment(Long productId, CommentRequestDto requestDto,
        UserDetailsImpl userDetails) { // 특정 상품에 댓글 작성
        Product product = findProduct(productId);
        Comment comment = commentRepository.save(
            new Comment(requestDto, product, userDetails.getUser()));
    ...
    public CommentResponseDto updateComment(Long commentId, CommentRequestDto requestDto,
        UserDetailsImpl userDetails) { // 댓글 수정
    ...
    private Product findProduct(Long productId) {
        return productRepository.findById(productId).orElseThrow(
            () -> new NotFoundException("해당 상품아이디는 존재하지 않습니다.")
        );
    }
    ...
    private Comment findComment(Long commentId, UserDetailsImpl userDetails) {
        ...
        if (!userDetails.getUser().getUserId().equals(comment.getUser().getUserId())) {
            throw new PermissionException("작성자만 삭제/수정이 가능합니다.");
        }
        return comment;
    }
